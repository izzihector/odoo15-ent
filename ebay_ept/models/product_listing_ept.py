...
class EbayProductListingEpt(models.Model):
    ...
    def _compute_get_time_remain(self):
        ...
        for cur_record in self:
            cur_record.time_remain_function = ""
    ...
    def create_variant_ebay_product_images(self, sku, ebay_pics, ebay_product):
        ...
        for ebay_var_spec_pic_set in ebay_variation_specific_picture_set:
            product_attr_value = product_attribute.value_ids.filtered(
                lambda x: x.attribute_id == product_attribute.id and x.name == ebay_var_spec_pic_set.get(
                    'VariationSpecificValue'))
            odo_product_id = ebay_product.product_id.filtered(
                lambda x: x.default_code == sku and x.attribute_value_ids == product_attr_value.id)
            if odo_product_id:
    ...
    def search_odoo_product_by_sku(self, sku):
        ...
        return product_product_obj.search([('default_code', '=', sku)])
    ...
    def sync_variation_product_listings(self, instance, item, variations, log_book_id, product_queue_line):
        """
        ...
        for variation in variations:
            ...
            if odoo_product and len(odoo_product) > 1:
                ...
                log_line = log_line_obj.create_log_lines(message, log_book_id.model_id.id, False, log_book_id)
    ...
    def sync_product_listings(self, queue_line, log_book_id):
        """
        This method is use to Sync product listing.
        @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 16 December 2021 .
        Task_id: 180925 - Import Products / variants
        """
        instance = queue_line.instance_id
        ...
        if product_listing:
            ...
            self.create_or_update_product_with_listings(instance, product_response, log_book_id, queue_line)
        _logger.info("End Processing ItemId: %s and name: %s" % ((product_response['ItemID']), product_response.get(
            'Title')))
        return True

    def create_or_update_product_with_listings(self, instance, product_response, log_book_id, queue_line):
        """
        ...
        if variations:
            ...
            self.sync_variation_product_listings(instance, product_response, variations, log_book_id, queue_line)
        elif product_response.get('SKU', False):
            self.create_or_update_product(product_response, instance, log_book_id, queue_line)
        else:
    ...
    def create_or_update_product(self, item, instance, log_book_id, product_queue_line):
        """
