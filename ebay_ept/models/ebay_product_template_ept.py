...
class EbayProductTemplateEpt(models.Model):
    ...
    def view_log_book(self):
        ...
        action_name = "ebay_ept.action_ebay_process_job_ept"
        form_view_name = "common_connector_library.action_common_log_book_ept_form"
        return process_import_export_obj.redirect_to_view(action_name, form_view_name, self.common_log_book_id.ids)
        ...
        if len(self.ebay_variant_ids.ids) > 1:
            ...
            for variant in self.ebay_variant_ids:
                stock = self.get_ebay_product_stock_ept(variant.instance_id, [variant.product_id.id],
                                                        variant.instance_id.warehouse_id)
    ...
    def prepare_variation_product_dict(self, ebay_product_template, instance, publish_in_ebay, schedule_time,
        ...
        for variant in ebay_product_template.ebay_variant_ids:
            stock = self.get_ebay_product_stock_ept(instance, [variant.product_id.id], instance.warehouse_id)
            variation = {}
            start_price = instance.pricelist_id.get_product_price_ept(variant.product_id)
            ...
            image_urls = self.prepare_ebay_product_images_dict(variant, instance, image_urls, log_book_id)
    ...
    def prepare_individual_item_dict(self, ebay_product_template, variant, instance, publish_in_ebay, schedule_time,
        ...
        list_image_url = self.prepare_ebay_product_images_dict(variant, instance, [], log_book_id)
        ...
        if site_id:
            ...
            stock_dict = self.get_ebay_product_stock_ept(instance, [variant.product_id.id], instance.warehouse_id)
    ...
    def prepare_ebay_product_images_dict(self, variant, instance, list_image_url, log_book_id=False):
        """
        ...
        for image in product_images:
            ...
            if ebay_image_url:
                ...
                log_line_obj.create_log_lines(message, log_book_id.model_id.id, variant.ebay_product_tmpl_id,
                                              log_book_id)
    ...
    def prepare_ebay_product_description_parameter(self, variant, ebay_product_template):
        ...
        product_description = (html.escape(description).encode("utf-8")).decode("iso-8859-1")
        if product_description == '<p><br></p>':
            product_description = ''
        ebay_product_template_id = ebay_product_template.ebay_template_id or False
        if ebay_product_template_id:
            product_description = "<![CDATA[%s]]>" % ebay_product_template_id.body_html
            product_description = product_description.encode("utf-8").decode("iso-8859-1")
    ...
    def prepare_ebay_product_prices_to_export(sub_dict, instance_config_pricelist_id, variant,
        ...
        Migration done by Haresh Mori @ Emipro on date 24 December 2021 .
        """
        product_price = instance_config_pricelist_id.get_product_price_ept(variant.product_id)
        _logger.info(
    ...
    def export_individual_item(self, ebay_product_template, publish_in_ebay, schedule_time, log_book_id):
        """
        ...
        if not product_listing:
            product_dict = self.prepare_individual_item_dict(ebay_product_template, ebay_variant, instance,
                                                             publish_in_ebay, schedule_time, log_book_id)
            ...
            try:
                ...
                log_line_obj.create_log_lines(str(error.response.dict()), log_book_id.model_id.id,
                                              ebay_product_template, log_book_id)
                log_book_id.message_post(body='Prepare dictionary data for export product:  ' + str(product_dict))
                return False
            if not results:
                results = {}
    ...
    def export_variation_item(self, ebay_product_template, publish_in_ebay, schedule_time, log_book_id):
        """
        ...
        if ebay_product_template.product_listing_ids.filtered(
            lambda x: x.instance_id == instance.id and x.state == 'Active'):
        ...
        try:
            ...
            if result.dict().get('Ack') not in ["Success", "Warning"]:
                log_line_obj.create_log_lines(error, log_book_id.model_id.id, ebay_product_template, log_book_id)
                log_book_id.message_post(body='Prepare dictionary data for export product:  ' + str(product_dict))
                return False
    ...
    def update_individual_item(self, ebay_product_template, publish_in_ebay, schedule_time, log_book_id):
        """
        ...
        if product_listing:
            ...
            try:
                ...
                log_line_obj.create_log_lines(str(error.response.dict()), log_book_id.model_id.id,
                                              ebay_product_template, log_book_id)
                log_book_id.message_post(body='Prepare dictionary data for update product:  ' + str(product_dict))
                return False
    ...
    def update_variation_item(self, ebay_product_template, publish_in_ebay, schedule_time, log_book_id):
        """
    ...
    def relist_products(self, ebay_product_template, instance, product_listing, log_book_id):
        """
        ...
        try:
            ...
            log_line_obj.create_log_lines(error, log_book_id.model_id.id, ebay_product_template, log_book_id)
            log_book_id.message_post(body='Prepare dictionary data for relist product:  ' + str(product_dict))
            return False
        if results.get('Ack', False) in ["Success", "Warning"]:
            variant_id = product_listing.ebay_variant_id.id
            self.create_ebay_product_listing(instance, results,
                                             product_listing.ebay_variant_id.ebay_product_tmpl_id,
    ...
    @staticmethod
    def get_values_for_product_listing(instance_id, item, ebay_product_tmpl_id, listing_type, variant_id=False):
        """
    ...
    def get_ebay_product_stock_ept(self, instance, product_ids, warehouse):
        ...
        if product_ids:
            if instance.ebay_stock_field == 'free_qty':
                ...
                product_listing_stock = product_obj.get_forecated_qty_ept(warehouse, product_ids)
