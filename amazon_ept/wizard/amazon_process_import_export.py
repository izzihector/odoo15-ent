...
class AmazonProcessImportExport(models.TransientModel):
    ...
    def create_and_request_amazon_vcs_report(self):
        ...
        vcs_report_vals.update({'report_type': '_SC_VAT_TAX_REPORT_',
                                'state': 'draft'})
    ...
    def amz_import_fbm_unshipped_orders(self):
        ...
        record.with_context(emipro_api='request_report_fbm_v13').request_report()
    ...
    def amz_fba_live_inventory_report(self):
        ...
        if self.amazon_program in ('pan_eu', 'cep'):
            ...
            report_type = '_GET_FBA_MYI_UNSUPPRESSED_INVENTORY_DATA_'
            fba_live_stock_report_ids = self.create_and_request_amazon_live_inv_report_ids(
    ...
    def amz_import_inbound_shipment(self):
        ...
        import_shipment_obj.get_inbound_import_shipment(self.instance_id, self.from_warehouse_id,
                                                        self.shipment_id, self.ship_to_address)
    ...
    def amz_shipment_report(self):
        ...
        shipment_report_tree_id = self.env.ref('amazon_ept.amazon_shipping_report_request_history_tree_view_ept')
        self.with_context({'raise_warning': True}).check_running_schedulers(
        ...
        if self.seller_id.is_another_soft_create_fba_shipment:
            fba_shipping_report_obj.process_and_create_amz_shipment_report(
            ...
            return {
                'name': _(fba_shipment_report_name),
                'view_mode': 'tree',
                'views': [[shipment_report_tree_id.id, 'list'],
                          [False, 'form']],
                'res_model': AMZ_SHIPPING_REPORT_REQUEST_HISTORY,
                'type': IT_ACTIONS_ACT_WINDOW,
                'domain': [('id', 'in', shipping_report_record_list)]
            }
        else:
    ...
    def get_amz_fba_shipping_report(self, start_date, end_date):
        ...
        shipping_report_record.with_context({'emipro_api': 'shipping_request_report_v13'}).request_report()
    ...
    def get_reports(self, vals):
        ...
        """
        tree_id = vals.get('tree_id', False)
        form_id = vals.get('form_id', False)
        ...
        kwargs.update({'report_type': vals.get('report_type', ''), 'start_date': start_date,
                       'end_date': end_date})
        ...
        odoo_report_ids = self.prepare_fba_report_vals(list_of_wrapper, vals.get('start_date', ''),
                                                       vals.get('end_date', ''), vals.get('model_obj', False),
                                                       vals.get('sequence', ''))
        ...
        for result in list_of_wrapper:
            ...
            for report in reports:
                ...
                if ReportType.GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2:
                    report_rec.with_context({'emipro_api': 'amazon_settlement_report_v13'}).get_report()
                else:
    ...
    def amz_import_products(self):
        ...
        amz_product_model_id = comman_log_line_obj.get_model_id(AMZ_PRODUCT_EPT)
        instance_dict = {}
        map_product_count = 0
        ...
        for line in reader:
            ...
            if not skip_line:
                ...
                if fulfillment not in ['FBA', 'FBM']:
                    ...
                    comman_log_line_obj.amazon_create_product_log_line(message, model_id, False, seller_sku,
                                                                       fulfillment, log_rec, mismatch=True)
                    continue
                ...
                if instance and fulfillment and seller_sku:
                    ...
                    if amazon_product_id:
                        ...
                        comman_log_line_obj.amazon_create_product_log_line(message, model_id, amazon_product_id.product_id,
                                                                           seller_sku, fulfillment, log_rec)
                        continue
                    ...
                    comman_log_line_obj.amazon_create_product_log_line(message, amz_product_model_id, product_id,
                                                                       seller_sku, fulfillment, log_rec)
        if not log_rec.log_lines:
            log_rec.unlink()
        message = ("%s Products Imported Successfully!" % map_product_count) if map_product_count else "All Products Skipped!!, Please check Log Book."
        ...
        if mismatch_logs:
            message = f'Some of the products are not imported you can check the logs: {mismatch_logs}'
    ...
    def create_amz_product_map_log(self):
        """
        This method will help to create product map log.
        ...
        log_vals = {
            'active': True,
            'type': 'import',
            'model_id': self.env['ir.model']._get(AMZ_PRODUCT_EPT).id,
            'res_id': self.id,
            'module': 'amazon_ept'
        }
    ...
    def read_import_csv_file_with_product_list(self, line, reader, model_id, log_rec, comman_log_line_obj):
        ...
        if not line.get(AMZ_SELLER_SKU):
            ...
            comman_log_line_obj.amazon_create_product_log_line(message, model_id, False, seller_sku,
                                                               fullfillment_by, log_rec, mismatch=True)
            skip_line = True
            ...
            comman_log_line_obj.amazon_create_product_log_line(message, model_id, False, seller_sku, fullfillment_by,
    ...
    def get_odoo_product_csv_data_ept(self, line_vals, fullfillment_by, log_rec):
        ...
        :param: log_rec : common.log.boo.ept() object
        return : odoo product.
        """
        ...
        if not product_id and self.auto_create_product:
            ...
            message = """ Odoo Product created for seller sku %s and Internal Reference %s """ % (
                seller_sku, odoo_default_code)
            comman_log_line_obj.amazon_create_product_log_line(message, model_id, product_id, seller_sku,
                                                               fullfillment_by, log_rec)
        if not product_id:
