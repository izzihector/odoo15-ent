...
class AmazonConfigSettings(models.TransientModel):
    ...
    stock_update_warehouse_ids = fields.Many2many(STOCK_WAREHOUSE, \
                                                  'stock_warehouse_amazon_config_rel',
                                                  'config_id', \
                                                  'warehouse_id', string="Stock Update Warehouses", \
    ...
    analytic_account_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic Tags', help='Analytic Tags')
    ...
    def onchange_amz_instance_id(self):
        ...
        if instance:
            ...
            values['analytic_account_id'] = instance.analytic_account_id.id if instance.analytic_account_id else False
            values['analytic_account_tag_ids'] = instance.analytic_account_tag_ids.ids if\
                instance.analytic_account_tag_ids else False
        else:
    ...
    def execute(self):
        ...
        if self.env.user.has_group('analytic.group_analytic_accounting') or\
                self.env.user.has_group('analytic.group_analytic_tags'):
            self.amz_seller_id.create_analytic_account_and_tag_records()
        ctx = {}
        if instance:
            ...
            values['analytic_account_tag_ids'] = self.analytic_account_tag_ids.ids if\
                self.analytic_account_tag_ids.ids else False
