...
class AmazonSellerEpt(models.Model):
    ...
    def create_analytic_account_and_tag_records(self):
        """
        Define method for create Analytic Account and Tags Records.
        """
        amazon_sellers = self.search([])
        if self.env.user.has_group('analytic.group_analytic_accounting') and amazon_sellers:
            self.create_analytic_account_and_group_records(amazon_sellers)
        if self.env.user.has_group('analytic.group_analytic_tags') and amazon_sellers.instance_ids:
            self.create_analytic_tag_records(amazon_sellers)
        return True

    def create_analytic_account_and_group_records(self, amazon_sellers):
        """
        Define method for create analytic account and group records.
        :param: amazon_sellers: list of amazon.seller.ept() objects
        :return: boolean
        """
        analytic_account_obj = self.env['account.analytic.account']
        analytic_account_group_obj = self.env['account.analytic.group']
        analytic_account_group = analytic_account_group_obj.search(
            [('name', '=', 'Amazon'), ('company_id', '=', self.env.user.company_id.id)], limit=1)
        if not analytic_account_group:
            analytic_account_group = analytic_account_group_obj.create({'name': 'Amazon'})
        for amazon_seller in amazon_sellers:
            analytic_account = analytic_account_obj.search([('name', '=', amazon_seller.name),
                                                            ('group_id', '=', analytic_account_group.id),
                                                            ('company_id', '=', self.env.user.company_id.id)], limit=1)
            if not analytic_account:
                analytic_account_obj.create({'name': amazon_seller.name, 'group_id': analytic_account_group.id})
        return True

    def create_analytic_tag_records(self, amazon_sellers):
        """
        Define method for create Analytic Account Tags records.
        :param: amazon_sellers: list of amazon.seller.ept() objects
        :return: boolean
        """
        analytic_account_tag_obj = self.env['account.analytic.tag']
        analytic_account_obj = self.env['account.analytic.account']
        amazon_analytic_tag = analytic_account_tag_obj.search([('name', '=', 'Amazon',)])
        if not amazon_analytic_tag:
            amazon_analytic_tag = analytic_account_tag_obj.create({'name': 'Amazon'})
        for amazon_seller in amazon_sellers:
            seller_analytic_account = analytic_account_obj.search([('name', '=', amazon_seller.name)], limit=1)
            seller_analytic_tag = analytic_account_tag_obj.search([('name', '=', amazon_seller.name)])
            if not seller_analytic_tag:
                seller_analytic_tag = analytic_account_tag_obj.create({'name': amazon_seller.name})
            tag_required_amazon_marketplaces = amazon_seller.instance_ids.filtered(
                lambda marketplace: not marketplace.analytic_account_tag_ids)
            if tag_required_amazon_marketplaces:
                self.create_and_set_analytic_tag_records(tag_required_amazon_marketplaces, amazon_analytic_tag,
                                                         seller_analytic_tag, seller_analytic_account)
        return True

    def create_and_set_analytic_tag_records(self, amz_marketplaces, amz_analytic_tag,
                                            seller_analytic_tag, seller_analytic_account):
        """
        Define method for create and set Analytic Tags as per Amazon Marketplaces
        :param: amz_marketplaces : amazon.instance.ept() objects - Amazon Marketplaces
        :param: amz_analytic_tag : account.analytic.tag() object - Amazon Analytic Tag
        :param: seller_analytic_tag : account.analytic.tag() object - Amazon Seller Analytic Tag
        :param: seller_analytic_account : account.analytic.account() object - Amazon Seller Analytic Account
        :return: boolean
        """
        analytic_account_tag_obj = self.env['account.analytic.tag']
        for amazon_marketplace in amz_marketplaces:
            marketplace_analytic_tag = analytic_account_tag_obj.search([('name', '=', amazon_marketplace.name)],
                                                                       limit=1)
            if not marketplace_analytic_tag:
                marketplace_analytic_tag = analytic_account_tag_obj.create({'name': amazon_marketplace.name})
            if not amazon_marketplace.analytic_account_id and seller_analytic_account:
                amazon_marketplace.write({'analytic_account_id': seller_analytic_account.id})
            amazon_marketplace.write({'analytic_account_tag_ids': (amz_analytic_tag
                                                                   + seller_analytic_tag
                                                                   + marketplace_analytic_tag).ids})
        return True
    ...
    def auto_import_fba_pending_sale_order_ept(self, args={}):
        ...
        if seller_id:
            ...
            sale_order_obj.with_context({'auto_process': True}).cancel_amazon_fba_pending_sale_orders(
                seller, marketplaceids, seller.instance_ids.ids)
        return True
    ...
    def auto_import_fba_shipment_status_ept(self, args={}):
        ...
        if seller_id:
            ...
            for amazon_result in items:
                shipment_id = amazon_result.get('ShipmentId').get('value', False)
                ...
                if amazon_inbound_shipments:
                        ...
                        for ship_member in ship_members:
                            if seller_sku == ship_member.get('SellerSKU').get('value', ''):
                                new_received_quantity = amazon_result.get('QuantityReceived').get(
                                    'value', 0.0)
                                old_quantity = ship_member.get('QuantityReceived').get('value', 0.0)
                    ...
                    amazon_inbound_shipments.update({shipment_id: [amazon_result]})
            if amazon_inbound_shipments:
                job_id = common_log_book_obj.create({'module': 'amazon_ept', 'type': 'import'})
                ...
                while max_range < len(ship_id_list):
                    ...
                    kwargs = self.prepare_fba_shipment_status_request_kwargs(seller)
                    kwargs.update({'emipro_api': 'check_status_v13',
                ...
                if not job_id:
                    ...
                    common_log_line_obj.create_log_lines(message, model_id, False, job_id)
    ...
    def request_fba_fulfilment_centers(self, sellers_ids=[]):
        ...
        sellers = self.env['amazon.seller.ept'].browse(sellers_ids)
        if not sellers:
            return False
