...
class InboundShipmentEpt(models.Model):
    ...
    def _compute_error_logs(self):
        ...
        model_id = common_log_line_obj.get_model_id(AMZ_INBOUND_SHIPMENT_EPT)
        logs = common_log_line_obj.search([('model_id', '=', model_id), ('res_id', '=', self.id)])
    ...
    def create_or_update_inbound_shipment(self, ship_plan, shipment, job=False):
        ...
        sku_qty_dict = self.env[INBOUND_SHIPMENT_PLAN_LINE].prepare_shipment_plan_line_dict(odoo_shipment, items)
        ...
        if create_or_update == 'update':
            ...
            kwargs.update({'emipro_api': 'update_shipment_in_amazon_v13',
            ...
            if response.get('reason', False):
                self.default_inbound_shipment_response_log(response, ship_plan, job)
                return False, job
        elif create_or_update == 'create':
            kwargs = self.prepare_inbound_shipment_kwargs(instance, odoo_shipment)
            kwargs.update({'emipro_api': 'create_shipment_in_amazon_v13',
                           'destination': fulfill_center_id,
                           'sku_qty_dict': sku_qty_dict,
                           'address_dict': address_dict,
                           'labelpreppreference': label_prep_type,
    ...
    def default_inbound_shipment_response_log(self, response, ship_plan, job=False):
        """
        Defautl log book for response of inbound shipments
        :param response:
        :param ship_plan:
        :param job:
        :return:
        ...
        if not job:
            job = log_obj.create({'module': 'amazon_ept', 'type': 'export'})
        log_line_obj.create({'message': error_value, 'res_id': ship_plan.id, 'log_book_id': job.id,
                             'model_id': log_line_obj.get_model_id(INBOUND_SHIPMENT_PLAN_EPT)})
    ...
    def amz_find_location_routes_ept(self, warehouse):
        ...
        if not location_routes:
            ...
            self.search_or_create_inbound_common_log_ept('export', AMZ_INBOUND_SHIPMENT_EPT, error_value)
    ...
    def search_or_create_inbound_common_log_ept(self, log_type, model_name, message):
        ...
        log_line_obj = self.env[COMMON_LOG_LINES_EPT]
        job = log_book_obj.search([('module', '=', 'amazon_ept'), ('res_id', '=', self.id),
        ...
        if not job:
            job = log_book_obj.create({'module': 'amazon_ept', 'type': log_type,
                                       ...
                                       'model_id': log_line_obj.get_model_id(model_name), 'res_id': self.id})
        log_line_obj.create({'message': message, 'model_id': log_line_obj.get_model_id(AMZ_INBOUND_SHIPMENT_EPT),
                             'res_id': self.id, 'log_book_id': job.id})
        return job
    ...
    def update_inbound_shipment_qty(self):
        ...
        for shipment in self:
            ...
            for x in range(0, len(shipment.odoo_shipment_line_ids), 20):
                ...
                sku_qty_dict = self.prepare_amz_sku_qty_list(shipment_lines, cases_required)
    ...
    def prepare_amz_sku_qty_list(shipment_lines, cases_required):
        ...
        for line in shipment_lines:
            ...
            sku_qty_dict.append({'sku': line.seller_sku, 'quantity': int(line.quantity),
                                 'quantity_in_case': int(line.quantity_in_case) if cases_required else 0})
        return sku_qty_dict
    ...
    def create_carton_contents_requests(self):
        ...
        for shipment in self:
            instance = self.get_instance(shipment)
            job_log_vals = {'type': 'export', 'module': 'amazon_ept'}
            ...
            for box, qty_dict in box_no_product_dict.items():
                if len(qty_dict.keys()) > 200:
                    ...
                    log_line_vals = {'model_id': log_line_obj.get_model_id(AMZ_INBOUND_SHIPMENT_EPT),
                                     'res_id': shipment.id or 0, 'message': log_message, 'log_book_id': job.id}
    ...
    def process_inbound_cartoon_content_request(self, header, data, instance, shipment):
        ...
        if data:
            ...
            kwargs.update({'emipro_api': 'create_carton_contents_requests_v13',
    ...
    def setup_partnered_ltl_data(self, parcel_data):
        ...
        """
        self.ensure_one()
        box_count = parcel_data.get('BoxCount', {}).get('value', False)
        ...
        is_bol_available = True if parcel_data.get('IsBillOfLadingAvailable', {}).get('value', '') == 'true' else False
        currency_code = parcel_data.get('AmazonCalculatedValue', {}).get('CurrencyCode', {}).get('value', '')
        vals = {}
        if currency_code:
        ...
        currency_value = parcel_data.get('AmazonCalculatedValue', {}).get('Value', {}).get('value', 0.0)
    ...
    def set_estimate_amount(self, parcel_data):
        ...
        deadline_date = parcel_data.get('PartneredEstimate', {}).get('VoidDeadline', {}).get('value', '')
        confirm_deadline_date = parcel_data.get('PartneredEstimate', {}).get('ConfirmDeadline', {}).get('value', '')
    ...
    def get_transport_content(self):
        ...
        kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
        kwargs.update({'emipro_api': 'get_transport_content_v13', 'shipment_id': self.shipment_id})
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
        if response.get('reason', False):
            raise UserError(_(response.get('reason', {})))
        ...
        is_partnered = True if transport_header.get('IsPartnered', {}).get('value', '') == 'true' else False
    ...
    def estimate_transport_request(self):
        ...
        instance = self.get_instance(self)
        kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
        kwargs.update({'emipro_api': 'estimate_transport_request_v13', 'shipment_id': self.shipment_id})
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
        if response.get('reason', False):
            error_value = response.get('reason', {})
    ...
    def get_carton_content_result(self):
        ...
        if self.feed_id:
            instance = self.get_instance(self)
            kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
            kwargs.update({'emipro_api': 'get_feed_submission_result_V13',
            ...
            try:
                ...
                if error != '0':
                    job_log_vals = {'module': 'amazon_ept', 'type': 'export'}
                    job = log_obj.create(job_log_vals)
                    ...
                    log_line_vals = {'model_id': log_line_obj.get_model_id(AMZ_INBOUND_SHIPMENT_EPT),
                                     'res_id': self.id or 0, 'message': description, 'log_book_id': job.id}
                    log_line_obj.create(log_line_vals)
    ...
    def export_partnered_small_parcel(self):
        ...
        log_obj = self.env[COMMON_LOG_BOOK_EPT]
        log_line_obj = self.env[COMMON_LOG_LINES_EPT]
        model_id = log_line_obj.get_model_id(AMZ_INBOUND_SHIPMENT_EPT)
        for shipment in self:
            instance = self.get_instance(shipment)
            job_log_vals = {'module': 'amazon_ept', 'type': 'export'}
            job = log_obj.create(job_log_vals)
            ...
            if not shipment.partnered_small_parcel_ids:
                ...
                log_line_obj.create_log_lines(message, model_id, shipment, job)
                continue
            data, skip_process = self.amz_prepare_partnered_small_parcel_data(shipment, data, model_id, job)
            if not skip_process:
                continue
            kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
            kwargs.update({'emipro_api': 'put_transport_content', 'data': data})
            response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
            if response.get('reason', False):
                error_value = response.get('reason', {})
    ...
    def amz_prepare_partnered_small_parcel_data(self, shipment, data, model_id, job):
        """
        ...
        for package in shipment.partnered_small_parcel_ids:
            ...
            if shipment.carrier_id:
                data.update({'TransportDetails.PartneredSmallParcelData.CarrierName':
                                 shipment.carrier_id.amz_carrier_code or shipment.carrier_id.name})
            data.update({'TransportDetails.PartneredSmallParcelData.PackageList.member.%d.Dimensions.Unit'
                         % (count): dimension_unit})
            data.update({'TransportDetails.PartneredSmallParcelData.PackageList.member.%d.Dimensions.Length'
                         % (count): str(package.ul_id.length)})
            data.update({'TransportDetails.PartneredSmallParcelData.PackageList.member.%d.Dimensions.Width'
                         % (count): str(package.ul_id.width)})
            data.update({'TransportDetails.PartneredSmallParcelData.PackageList.member.%d.Dimensions.Height'
                         % (count): str(package.ul_id.height)})
            data.update({'TransportDetails.PartneredSmallParcelData.PackageList.member.%d.Weight.Unit'
                         % (count): package.weight_unit})
            data.update({'TransportDetails.PartneredSmallParcelData.PackageList.member.%d.Weight.Value'
    ...
    def export_non_partnered_small_parcel_tracking(self):
        ...
        for shipment in self:
            ...
            traking_dict = self.prepare_non_partnered_small_parcel_data(pickings, shipment, job, model_id)
            data.update(traking_dict)
            kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
            kwargs.update({'emipro_api': 'export_non_partnered_small_parcel_tracking_v13', 'data': data})
            response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
            self.process_small_parcel_tracking_response_result(response, pickings, shipment, model_id, job)
            if shipment.state != 'ERROR':
                self.update_shipment_ept(shipment, pickings, back_orders, auto_called, instance, job)
        return True

    def process_small_parcel_tracking_response_result(self, response, pickings, shipment, model_id, job):
        """
        ...
        :param shipment: amazon.inbound.shipment.ept()
        :param model_id: ir.model id
        :param job: common logbook object
        :return:
        """
        log_line_obj = self.env[COMMON_LOG_LINES_EPT]
    ...
    def prepare_non_partnered_small_parcel_data(self, pickings, shipment, job, model_id):
        """
        Prepare data for update non partnered small parcel transport.
        :param pickings: stock.picking()
        :param shipment: amazon.inbound.shipment.ept()
        :param job: common logbook
        :param model_id: ir.model
        ...
        for picking in pickings:
            for op in picking.move_line_ids:
                ...
                if tracking_no and tracking_no not in tacking_no_list:
                    count_box += 1
                    traking_dict.update({
                        'TransportDetails.NonPartneredSmallParcelData.PackageList.member.%s.TrackingId'
                        % str(count_box): str(tracking_no)})
        ...
        while True:
            if count_box >= shipment.box_count:
                break
            count_box += 1
            traking_dict.update({'TransportDetails.NonPartneredSmallParcelData.PackageList.member.%s.TrackingId'
                                 % str(count_box): str(' ')})
        return traking_dict

    def update_shipment_ept(self, fba_shipment, pickings, back_orders, auto_called, instance, job=False):
        """
        ...
        for picking in back_orders:
            self.amz_update_shipment_back_orders(fba_shipment, picking, auto_called, instance, sku_list, job)
        pickings.write({'shipment_status': 'SHIPPED'})
        return True

    def amz_update_shipment_back_orders(self, fba_shipment, picking, auto_called, instance, sku_list, job=False):
        """
    ...
    def update_shipment_in_amazon(self, picking, sku_qty_dict, instance, fba_shipment, job, auto_called):
        """
        ...
        if response.get('reason', False):
            error_value = response.get('reason', {})
            if job:
                log_line_obj.create_log_lines(error_value, model_id, fba_shipment, job)
            else:
                if not auto_called:
                    raise UserError(_(error_value))
    ...
    def confirm_transport_request(self):
        ...
        instance = self.get_instance(self)
        kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
        kwargs.update({'emipro_api': 'confirm_transport_request_v13', 'shipment_id': self.shipment_id})
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
        if response.get('reason', False):
            error_value = response.get('reason', {})
            raise UserError(_(error_value))
    ...
    def void_transport_request(self):
        ...
        if not self.void_deadline_date:
            self.get_transport_content()
        kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
        kwargs.update({'emipro_api': 'void_transport_request_v13', 'shipment_id': self.shipment_id})
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
        if response.get('reason', False):
            error_value = response.get('reason', {})
            raise UserError(_(error_value))
    ...
    def check_status(self):
        ...
        for instance, shipment_ids in instance_shipment_ids.items():
            ...
            response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
            if response.get('reason', {}) and self._context.get('is_auto_process', False):
                self.search_or_create_inbound_common_log_ept('import', AMZ_INBOUND_SHIPMENT_EPT,
                                                             response.get('reason', {}))
            elif response.get('reason', False):
                raise UserError(_(response.get('reason', {})))
            amazon_shipments = response.get('amazon_shipments', False) or []
    ...
    def amz_check_status_process_shipments(self, amazon_shipments, instance):
        ...
        for ship_member in amazon_shipments:
            flag = False
            cases_required = bool(ship_member.get('AreCasesRequired', {}).get('value', '') == 'true')
            ...
            if shipment_status in ['RECEIVING', 'CLOSED']:
                kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
                kwargs.update({'emipro_api': 'check_amazon_shipment_status_v13', 'amazon_shipment_id': shipmentid})
                response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
                if response.get('reason', False):
                    raise UserError(_(response.get('reason', {})))
                ...
                if shipment_status == 'CLOSED':
                    ...
                    if odoo_shipment_rec:
                        pickings = odoo_shipment_rec.mapped('picking_ids').filtered(
                            lambda r: r.state in ['partially_available', 'assigned'] and r.is_fba_wh_picking)
                    if pickings:
    ...
    def get_remaining_qty(self, response, instance, amazon_shipment_id, odoo_shipment_rec):
        ...
        # Assign Stock to the picking and process it
        self.assign_and_process_new_pickings(new_picking)
        return True
    ...
    def check_status_ept(self, amazon_shipments, seller, job_id):
        """
            ...
            for key, amazon_shipment in amazon_shipments.items():
                ...
                if shipment:
                    ...
                    if pickings:
                        ...
                        self.amz_process_remaining_qty(pickings, shipment, shipmentid, amazon_shipment, model_id, job_id)
                else:
                    message = "Shipment %s is not found in ERP" % (shipmentid)
                    log_line_obj.create_log_lines(message, model_id, '', job_id)
        return True

    def amz_process_remaining_qty(self, pickings, shipment, shipmentid, amazon_shipment, model_id, job_id):
        """
        :param pickings:
        :param shipment:
        :param shipmentid:
        ...
        if not pickings:
            ...
            log_line_obj.create_log_lines(message, model_id, shipment, job_id)
        ...
        if picking:
            for item in items:
                ...
                if not amazon_product:
                    ...
                    log_line_obj.create_log_lines(message, model_id, odoo_shipment_rec, job_id)
                    continue
                ...
                done_moves = odoo_shipment_rec.picking_ids.filtered(
                    lambda r: r.amazon_shipment_id == amazon_shipment_id and r.is_fba_wh_picking).mapped(
                        'move_lines').filtered(lambda r, odoo_product=odoo_product: r.state == 'done' and
                ...
                if received_qty <= 0.0:
                    continue
                if not new_picking:
                    new_picking = picking.copy({'is_fba_wh_picking': True, 'move_lines': [], 'group_id': False,
                                                'location_id': picking.location_id.id,
                                                'location_dest_id': picking.location_dest_id.id})
                move = picking.move_lines[0]
    ...
    def assign_and_process_new_pickings(self, new_picking):
        stock_immediate_transfer_obj = self.env['stock.immediate.transfer']
        if new_picking:
            new_picking.action_confirm()
            new_picking.action_assign()
            stock_immediate_transfer_obj.with_context(default_pick_ids=[(4, new_picking.id)],
                                                      button_validate_picking_ids=[new_picking.id]).create({}).process()
    ...
    def update_non_partnered_carrier(self):
        ...
        data = {'ShipmentId': self.shipment_id,
                'IsPartnered': str(bool(self.is_partnered)).lower(),
                'ShipmentType': 'SP' if self.shipping_type == 'sp' else 'LTL'}
        if self.shipping_type == 'sp':
            data.update({'TransportDetails.NonPartneredSmallParcelData.CarrierName': 'OTHER',
                         'TransportDetails.NonPartneredSmallParcelData.PackageList.member.'+str('1')+'.TrackingId': str(' '),})
        else:
            carrier_code = self.carrier_id.amz_carrier_code if self.carrier_id else 'OTHER'
            data.update({'TransportDetails.NonPartneredLtlData.ProNumber': '##########',
    ...
    def export_partnered_ltl_parcel(self):
        ...
        for shipment in self:
            ...
            if len(shipment.partnered_ltl_ids.ids) <= 0:
                ...
                log_line_obj.create_log_lines(message, model_id, shipment, job)
                continue
            data, flag = self.prepare_partnered_ltl_parcel_data(shipment, model_id, job)
            ...
            self.process_inbound_partnered_ltl_shipment(shipment, data, flag, model_id, job)
        return True

    def process_inbound_partnered_ltl_shipment(self, shipment, data, flag, model_id, job):
        """
    ...
    def prepare_partnered_ltl_parcel_data(self, shipment, model_id, job):
        """
        ...
        if shipment.seller_freight_class:
            data.update({'TransportDetails.PartneredLtlData.SellerFreightClass': shipment.seller_freight_class})
        count = 1
        flag = True
        for pallet in shipment.partnered_ltl_ids:
            ...
            if pallet.weight_unit and pallet.weight_value:
                data.update({
                    'TransportDetails.PartneredLtlData.PalletList.member.%d.Weight.Unit' % (count): pallet.weight_unit})
                data.update({
            ...
            data.update({'TransportDetails.PartneredLtlData.PalletList.member.%d.IsStacked' % (
                count): str(bool(pallet.is_stacked)).lower() })
            count += 1
        return data, flag
    ...
    def get_bill_of_lading(self):
        ...
        instance = self.get_instance(self)
        kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
        kwargs.update({'emipro_api': 'get_bill_of_lading_v13', 'shipment_id': self.shipment_id})
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
        if response.get('reason', False):
            raise UserError(_(response.get('reason', {})))
        file_path = '/tmp/bill_of_landing.zip'
        result = response.get('result', {})
        bol_doc = result.get('TransportDocument', {}).get('PdfDocument', {}).get('value', '')
        if bol_doc:
            bol_doc = base64.b64decode(bol_doc)
            bol_zip = open(file_path, 'wb')
