...
class SettlementReportEpt(models.Model):
    ...
    def reconcile_other_transaction_line_with_taxes(self, tax_id, line_id, move_line_list, account_id,
        ...
        for move_tax_data in mv_line_tax_list:
            ...
            vt_line_tag_ids = move_tax_data.get('tax_tag_ids', [])
    ...
    def prepare_amazon_request_report_kwargs(self, seller):
        """
        Inherited this method to update the  settlement report emipro api
        """
        kwargs = super(SettlementReportEpt, self).prepare_amazon_request_report_kwargs(seller)
        kwargs.update({'emipro_api': 'amazon_settlement_report_v13', 'name': self.name})
        return kwargs
    ...
    def create_refund_invoices(self, refund_list_item_price, bank_statement):
        ...
        for order_key, product_amount in refund_list_item_price.items():
            ...
            if not refund_exist:
                ...
                if not invoices:
                    continue
                # Get those all invoices which invoiced quantity is not zero in the sale order lines
                # for the same products
                invoices = invoices.invoice_line_ids.sale_line_ids.filtered(
                    lambda sale_line: sale_line.qty_invoiced != 0.0 and sale_line.product_id.id in product_ids
                ).mapped('invoice_lines').mapped('move_id')
                if not invoices:
                    continue
    ...
    def auto_process_settlement_report(self, args={}):
        ...
        if seller_id:
            ...
            if seller:
                settlement_reports = self.search(
                    [('seller_id', '=', seller.id), ('state', 'in', ['_DONE_', 'imported']),
                     ('report_id', '!=', False)])
