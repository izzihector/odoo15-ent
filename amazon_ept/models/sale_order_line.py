...
class SaleOrderLine(models.Model):
    ...
    def create_sale_order_line_vals_amazon(self, order_line, odoo_product, amazon_order):
        ...
        if amazon_order.amz_instance_id.analytic_account_tag_ids:
            order_vals = self.set_analytic_tags_ept(amazon_order.amz_instance_id, order_vals)
        return order_vals

    @staticmethod
    def set_analytic_tags_ept(amz_instance, order_line_values):
        """
        Define method for set Analytic Account Tags in Sale order line as per Amazon order Marketplace.
        :param: amz_instance : amazon.instance.ept() object
        :param: order_line_values : sale order line values dict {}
        :return: update dict{}
        """
        order_line_values.update({'analytic_tag_ids': amz_instance.analytic_account_tag_ids.ids})
