...
class SaleOrder(models.Model):
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        :return : account.move()
        @author : kishan Sorani on date 02-Nov-2021
        """
    ...
    def import_fba_pending_sales_order(self, seller, marketplaceids, updated_after_date):
        ...
        kwargs = self.prepare_amazon_request_report_kwargs(seller, 'import_fba_pending_sales_order_v13')
    ...
    def process_fba_pending_sale_order_response(self, seller, kwargs, result):
        ...
        for order in order_by_next_token:
            self.create_amazon_pending_sales_order(seller, [order])
            self._cr.commit()
        return True
    ...
    def cancel_amazon_fba_pending_sale_orders(self, seller, marketplaceids, instance_ids):
        ...
        for from_date, to_date in list(date_ranges.items()):
            ...
            list_of_wrapper = self.check_amazon_fba_and_fbm_cancel_order(seller, marketplaceids, created_after, 'AFN')
    ...
    def cancel_amazon_fbm_pending_sale_orders(self, seller, marketplaceids, instance_ids):
        ...
        if unshipped_orders:
            ...
            list_of_wrapper = self.check_amazon_fba_and_fbm_cancel_order(seller, marketplaceids, updated_after_date,
                                                                         'MFN')
            for result in list_of_wrapper:
    ...
    def check_amazon_fba_and_fbm_cancel_order(self, seller, marketplaceids, updated_after,
        ...
        """
        kwargs = self.prepare_amazon_request_report_kwargs(seller, 'check_cancel_order_in_amazon_v13')
    ...
    def check_and_cancel_amazon_orders(self, seller, fulfillment_by, orders):
        ...
        model_id = comman_log_line_obj.get_model_id(SALE_ORDER)
        ...
        for order in orders:
            ...
            if not existing_order:
                ...
                comman_log_line_obj.amazon_create_order_log_line(message, model_id, False, amazon_order_ref,
                                                                 False, fulfillment_by, log_rec)
                continue
            if existing_order and existing_order.state != 'draft':
                ...
                comman_log_line_obj.amazon_create_order_log_line(message, model_id, False, existing_order.name,
                                                                 False, fulfillment_by, log_rec)
                continue
    ...
    def create_amazon_pending_sales_order(self, seller, list_of_wrapper):
        ...
        amazon_order_list = []
        product_details = {}
        model_id = comman_log_line_obj.get_model_id(AMZ_SHIPPING_REPORT_REQUEST_HISTORY)
        log_rec = log_book_obj.amazon_create_transaction_log('import', model_id, self.id)
        for wrapper_obj in list_of_wrapper:
            orders = []
            if not isinstance(wrapper_obj.get('Orders', {}).get('Order', []), list):
            ...
            for order in orders:
                ...
                if existing_order:
                    ...
                    comman_log_line_obj.amazon_create_order_log_line(message, model_id, self.id, amazon_order_ref,
                                                                     False, 'FBA', log_rec)
                    continue
    ...
    def request_fba_pending_sale_order_and_process_ept(self, seller, instance, order,
        ...
        for order_line_wrapper_obj in list_of_orderlines_wrapper:
            if not isinstance(order_line_wrapper_obj.get('OrderItems', {}).get('OrderItem', []), list):
                order_lines.append(
            ...
            for order_line in order_lines:
                skip_order, product_details = self.with_context(
                    {'is_fba_order': True}).create_or_find_amazon_product(order_line, product_details, instance,
                                                                          amazon_order_ref, log_rec)
                if skip_order:
                    break
    ...
    def prepare_fba_pending_order_line_ept(order_line):
        ...
        return {
            ...
            'line_tax_amount': float(order_line.get('ItemTax', {}).get('Amount', {}).get('value', 0.0)),
            'amazon_order_item_id': order_line.get('OrderItemId', {}).get('value', False),
            ...
            'shipping_charge': float(order_line.get('ShippingPrice', {}).get('Amount', {}).get('value', 0.0)),
            'shipping_tax': float(order_line.get('ShippingTax', {}).get('Amount', {}).get('value', 0.0)),
            'gift_wrapper_charge': float(order_line.get('GiftWrapPrice', {}).get('Amount', {}).get('value', 0.0)),
            'gift_wrapper_tax': float(order_line.get('GiftWrapTax', {}).get('Amount', {}).get('value', 0.0)),
    ...
    def prepare_amazon_sale_order_vals(instance, partner_dict, order):
        ...
        if order.get('PurchaseDate', {}).get('value', False):
            date_order = parser.parse(
                order.get('PurchaseDate', {}).get('value', False)).astimezone(utc).strftime(DATE_YMDHMS)
        else:
    ...
    def check_already_status_updated_in_amazon(self, seller, marketplaceids, instances, next_token={}):
        ...
        if updated_after_date:
            ...
            start_date = time.strftime(DATE_YMDTHMS, time.gmtime(
                time.mktime(time.strptime(db_import_time, DATE_YMDTHMS))))
            updated_after_date = str(start_date) + 'Z'
        kwargs = self.prepare_amazon_request_report_kwargs(seller, 'check_already_status_updated_in_amazon_ept_v13')
        ...
        next_token_wrapper = list_of_wrapper[-1].get('NextToken', {}).get('value', {}) \
            if list_of_wrapper and isinstance(list_of_wrapper, list) else {}
    ...
    def create_or_find_amazon_product(self, order_details, product_details, instance,
        ...
        if not amazon_product:
            odoo_product, skip_order = self.search_or_create_odoo_product(instance, amazon_order_ref, fulfillment_by,
                                                                          order_details, skip_order, log_book)
            if not skip_order:
    ...
    def search_or_create_odoo_product(self, instance, amazon_order_ref, fulfillment_by,
                                      order_details, skip_order, log_book):
        """
        ...
        ir_model_obj = self.env[IR_MODEL]
        product_model_id = ir_model_obj._get('product.product').id
        model_id = ir_model_obj._get(AMZ_PRODUCT_EPT).id
        seller_sku = order_details.get('SellerSKU', {}).get('value', '')
        ...
        if odoo_product:
            ...
            # Create Odoo Product
            erp_prod_vals = {'name': order_details.get('Title', {}).get('value', ''),
    ...
    def prepare_amazon_customer_vals(row):
        ...
        return {
            ...
            'StateOrRegion': row.get('ShippingAddress', {}).get('StateOrRegion', {}).get(
                'value', '') if row.get('ShippingAddress', {}).get('StateOrRegion', '') else None,
            ...
            'AmazonOrderId': row.get(
                'AmazonOrderId', {}).get('value', '') if row.get('AmazonOrderId', False) else None,
        }
    ...
    def amz_create_order_lines(self, order, instance, order_details, dict_product_details):
        ...
        if instance.analytic_account_tag_ids:
            order_line_vals = order.order_line.set_analytic_tags_ept(instance, order_line_vals)
    ...
    def get_fbm_shipped_order_line(self, instance, order, order_details):
        ...
        if order_details.get('ShippingPrice', False) and float(
            ...
            ship_line_vals.update({
                'amazon_order_item_id': order_details.get('OrderItemId', {}).get('value', False) + "_ship",
            ...
            # Set Analytic Tags in Sale order line from Amazon Marketplace
            if instance.analytic_account_tag_ids:
                ship_line_vals = order.order_line.set_analytic_tags_ept(instance, ship_line_vals)
    ...
    def get_fbm_shipped_discount_order_line(self, instance, order, order_details):
        ...
        if order_details.get('ShippingDiscount', False) and float(
                order_details.get('ShippingDiscount', {}).get('Amount', {}).get(
                    'value', 0.0)) < 0 and instance.seller_id.ship_discount_product_id:
    ...
    def get_fbm_promotion_discount_line(self, instance, order, order_details):
        ...
        if order_details.get('PromotionDiscount', False) and float(
                order_details.get('PromotionDiscount', {}).get('Amount', {}).get(
                    'value', 0.0)) > 0 and instance.seller_id.promotion_discount_product_id:
            item_discount = float(order_details.get('PromotionDiscount', {}).get('Amount', {}).get(
                'value', 0.0)) * (-1)
            discount_tax = float(order_details.get('PromotionDiscountTax', {}).get('Amount', {}).get(
                'value', 0.0))
    ...
    def create_fbm_shipped_chargable_order_line(order, instance, product_id, price_unit):
        ...
        # Set Analytic Tags in Sale order line from Amazon Marketplace
        if instance.analytic_account_tag_ids:
            chargable_vals = order.order_line.set_analytic_tags_ept(instance, chargable_vals)
    ...
    def prepare_amazon_request_report_kwargs(self, seller_id, emipro_api):
        ...
        return {'merchant_id': str(seller_id.merchant_id) if seller_id.merchant_id else False,
                'auth_token': str(seller_id.auth_token) if seller_id.auth_token else False,
                'app_name': 'amazon_ept',
                'account_token': account.account_token,
    ...
    def amz_create_sales_order(self, queue_order, log_book):
        ...
        request_counter = 0
        model_id = self.env[IR_MODEL]._get(SALE_ORDER).id
        ...
        for line in queue_lines:
            ...
            if not order.get('PurchaseDate', {}).get('value', ''):
                ...
                line.state = 'failed'
                continue
            if not order.get('BuyerEmail', {}).get('value', ''):
    ...
    def process_shipped_or_missing_unshipped_lines_ept(self, instance, order, line,
        ...
        for order_line_wrapper_obj in list_of_shipped_order_lines:
            order_lines = []
            if not isinstance(order_line_wrapper_obj.get('OrderItems', {}).get('OrderItem', []), list):
                order_lines.append(order_line_wrapper_obj.get('OrderItems', {}).get('OrderItem', {}))
            else:
                order_lines = order_line_wrapper_obj.get('OrderItems', {}).get('OrderItem', [])
            ...
            if sales_order:
                ...
                if order_status == 'Shipped':
                    ...
                    common_log_line_ept.amazon_create_order_log_line(
                        message, model_id, False, sales_order.amz_order_reference, False, 'FBM', log_book)
    ...
    def process_shipped_or_missing_unshipped_order_ept(self, instance, order, order_lines, line_state, log_book):
        ...
        for order_line in order_lines:
            skip_order, dict_product_details = self.create_or_find_amazon_product(order_line, dict_product_details,
                                                                                  instance, amazon_order_ref, log_book)
            if skip_order:
                line_state = 'failed'
                break
        ...
        for data in datas:
            ...
            if not instance:
                instance = seller.mapped('instance_ids').filtered(
                    lambda l, data=data: l.marketplace_id.name == data.get('SalesChannel', {}).get('value', ''))
                instance_dict.update({data.get('SalesChannel', {}).get('value', ''): instance})
            shipped_order_data_queue_line_obj.create({
                'order_id': data.get('AmazonOrderId', {}).get('value', False),
    ...
    def create_amazon_sales_order_vals(self, partner_dict, order, instance):
        ...
        vals.update({'picking_policy': instance.seller_id.fba_auto_workflow_id.picking_policy})
    ...
    def process_amazon_update_tracking_feed_response(self, results, data, seller, shipment_pickings):
        ...
        if results.get('FeedSubmissionInfo', {}).get('FeedSubmissionId', {}).get('value', False):
            ...
            feed_submission_id = results.get('FeedSubmissionInfo', {}).get(
                'FeedSubmissionId', {}).get('value', False)
    ...
    def get_amazon_customer_vat_details(self, order, vat, vat_country_code):
        """
        Find Out VAT and Vat Country from OrderItem Response.
        return vat and vat country code
        """
        ...
        order.update({'check_vat_ept': bool(vat_module)})
    ...
    def get_amazon_tax_registration_details(self, order):
        ...
        if order.get('BuyerEmail', {}).get('value'):
            ...
            if tax_registration_details:
                vat, vat_country_code = self.get_amazon_customer_vat_details(order, vat, vat_country_code)
            else:
