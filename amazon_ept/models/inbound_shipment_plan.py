...
class InboundShipmentPlanEpt(models.Model):
    ...
    def _compute_error_logs(self):
        common_log_line_ept_obj = self.env[COMMON_LOG_LINES_EPT]
        model_id = common_log_line_ept_obj.get_model_id(INBOUND_SHIPMENT_PLAN_EPT)
    ...
    def cancel_entire_inbound_shipment(self, shipment, job):
        ...
        if response.get('reason', False):
            ...
            log_line_obj.create_log_lines(error_value, model_id, self, job)
        else:
    ...
    def create_inbound_shipment_plan(self):
        ...
        while True:
            ...
            shipment_lines = self.mapped('shipment_line_ids').filtered(
                lambda r, already_taken=already_taken: r.odoo_shipment_id is not set and r.id not in already_taken)
            if not shipment_lines:
                break
            if len(shipment_lines.ids) > 15:
                shipment_line_ids = shipment_lines[x:x + 15]
            else:
            ...
            for shipment_line in self.shipment_line_ids:
                if self.is_are_cases_required:
                    ...
                    sku_qty_dict.append({'sku': shipment_line.seller_sku, 'quantity': int(shipment_line.quantity)})
            ...
            kwargs.update({'ship_to_country_code': ship_to_country_code, 'labelpreppreference': self.label_preference,
                           'sku_qty_dict': sku_qty_dict, 'ship_from_address': address_dict})
            response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
            if response.get('reason', False):
                error_value = response.get('reason', {})
                if not job:
            ...
            if not isinstance(result.get('InboundShipmentPlans', {}).get('member', []), list):
