...
class AmazonRemovalOrderReportHistory(models.Model):
    ...
    def amz_removal_search_or_create_job(self):
        ...
        @author: Keyur Kanani
        :Migration done by kishan sorani on date 30-Sep-2021
        """
        amazon_process_job_log_obj = self.env[COMMON_LOG_BOOK_EPT]
        model_id = self.env[IR_MODEL]._get(AMZ_REMOVAL_ORDER_REPORT_HISTORY).id
        job = amazon_process_job_log_obj.amazon_search_or_create_transaction_log('import', model_id, self.id)
        if job.log_lines:
    ...
    def process_removal_order_report(self):
        ...
        is_mismatch_logs = bool(job.log_lines.filtered(lambda l: l.mismatch_details))
    ...
    def amz_prepare_disposal_and_removal_line_dict(self, existing_order, rows, job,
                                                   disposal_line_dict, return_line_dict, liquidations_line_dict):
        """
        ...
        for row in rows:
            ...
            if not amazon_removal_order_config:
                ...
                disposal_line_dict, return_line_dict, liquidations_line_dict = self.update_return_or_removal_line_dict_ept(
                    key, row, disposal_line_dict, return_line_dict, job, liquidations_line_dict)
    ...
    def create_order_if_not_found_in_odoo(self, order_dict, job, disposal_line_dict,
        ...
        for order_id, rows in list(order_dict.items()):
            ...
            if len(rows) == skip_lines:
                continue
            if lines:
                removal_order = self.create_removal_order_and_process_pickings(order_id, order_type, instance,
                                                                               lines, job)
                disposal_line_dict, return_line_dict, liquidations_line_dict = self.amz_prepare_disposal_and_removal_line_dict(
    ...
    @staticmethod
    def update_return_or_removal_line_dict_ept(key, row, disposal_line_dict, return_line_dict, job, liquidations_line_dict):
        """
    ...
    def create_removal_order_and_process_pickings(self, order_id, order_type, instance, lines, job):
        """
        ...
        removal_vals = self.prepare_amz_removal_order_vals_ept(order_id, order_type, instance, lines, job)
    ...
    def prepare_amz_removal_order_vals_ept(self, order_id, order_type, instance, lines, job):
        """
    ...
    def process_disposal_lines(self, disposal_line_dict, job):
        ...
        for order_key, rows in list(disposal_line_dict.items()):
            ...
            for row in rows:
                ...
                product = self.find_amazon_product_for_process_removal_line(row, job, order.instance_id.id)
                if product:
    ...
    def process_return_lines(self, return_line_dict, job):
        ...
        for order_key, rows in list(return_line_dict.items()):
            ...
            for row in rows:
                ...
                if shipped_qty > 0.0:
                    move_pickings, skip_lines = self.amz_removal_procesed_qty_ept(
                        row, picking_vals, shipped_qty, job)
                    if skip_lines:
                        continue
                    if move_pickings:
                        pickings += move_pickings
