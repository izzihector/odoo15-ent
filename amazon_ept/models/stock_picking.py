...
class StockPicking(models.Model):
        ...
        for picking in pickings:
            ...
            for item in items:
                ...
                done_moves = picking.odoo_shipment_id.picking_ids.filtered( \
                    ...
                    'move_lines').filtered(
                    lambda r, odoo_product_id=odoo_product_id: r.product_id.id == odoo_product_id and
                                                               r.state == 'done')
        ...
        if pickings:
            ...
            for item in items:
                ...
                done_moves = odoo_shipment_id.picking_ids.filtered(
                    lambda r: r.is_fba_wh_picking and r.amazon_shipment_id == amazon_shipment_id).mapped('move_lines'). \
                    filtered(
            ...
            if return_picking:
                ...
                stock_immediate_transfer_obj.with_context(
    ...
    def amz_inbound_get_stock_move_ept(picking, odoo_product_id):
        ...
        :return: stock.move()
        """
        stock_move = picking.move_lines.filtered(lambda r: r.product_id.id == odoo_product_id.id and \
    ...
    def amz_inbound_shipment_plan_line_ept(self, odoo_shipment_id, amazon_product, item):
        ...
        asin = item.get('FulfillmentNetworkSKU', {}).get('value', '')
        shipped_qty = item.get('QuantityShipped', {}).get('value', 0.0)
        received_qty = float(item.get('QuantityReceived', {}).get('value', 0.0))
    ...
    def update_shipment_quantity(self):
        ...
        for picking in self:
            ...
            label_prep_type = self.get_amazon_label_prep_type(odoo_shipment, ship_plan)
            for x in range(0, len(picking.move_lines), 20):
                move_lines = picking.move_lines[x:x + 20]
                sku_qty_dict = {}
                for move in move_lines:
                    amazon_product = self.get_inbound_shipment_line_product(move, instance)
                    line = plan_line_obj.search([('odoo_shipment_id', '=', odoo_shipment.id),
                                                 ('amazon_product_id', 'in', amazon_product.ids)])
                    sku_qty_dict.update({str(line and line.seller_sku or amazon_product[0].seller_sku):
                                             str(int(move.reserved_availability))})
                    kwargs = odoo_shipment.prepare_inbound_shipment_kwargs(instance, odoo_shipment)
    ...
    def check_qty_difference_and_create_return_picking(self, response, amazon_shipment_id, odoo_shipment_id, instance):
        """
        ...
        """
        stock_immediate_transfer_obj = self.env['stock.immediate.transfer']
        return_picking = self.amz_process_inbound_return_picking_ept(response, instance, odoo_shipment_id,
                                                                     amazon_shipment_id)
        if return_picking:
            return_picking.action_confirm()
            return_picking.action_assign()
    ...
    def amz_process_inbound_return_picking_ept(self, response, instance, odoo_shipment_id, amazon_shipment_id):
        """
            ...
            if received_qty < 0.0:
                ...
                for move in done_moves:
                    if move.product_qty <= received_qty:
                        ...
                        return_qty = received_qty
                    self.amz_copy_stock_move_data_ept(move, return_picking, received_qty, pickings, pick_type_id)
                    received_qty = received_qty - return_qty
                    if received_qty <= 0.0:
                        break
