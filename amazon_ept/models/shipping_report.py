...
class ShippingReportRequestHistory(models.Model):
    ...
    def default_get(self, fields):
        ...
        if fields:
            res.update({'report_type': ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA})
        return res
    ...
    def process_amazon_shipment_orders(self, outbound_orders_dict, order_details_dict_list, sale_order_list, log_rec):
        ...
        if outbound_orders_dict:
            if self.seller_id.amz_fba_us_program == 'narf':
                ...
                self.process_pan_eu_outbound_orders(outbound_orders_dict, log_rec)
            else:
    ...
    def search_amazon_outbound_order_ept(self, order_ref, job):
        ...
        if not amazon_order:
            ...
            common_log_line_obj.amazon_create_order_log_line(message, model_id, self.id, order_ref, False, 'FBA',
                                                             job, mismatch=True)
            return amazon_order
        if amazon_order.filtered(lambda x: x.state in ["draft", "sent"] and x.seller_id.amz_fba_us_program == 'narf'):
            amazon_order.action_confirm()
        return amazon_order
    ...
    def attach_amazon_data(self, order_lines, order_line_data):
        ...
        for data in order_line_data:
            order_line = order_lines.filtered(
                lambda x, data=data: x.amazon_order_item_id == data.get("amazon-order-item-id", False) and
    ...
    def check_and_update_vat_ept(self, row, instance, country_obj, vat_country, ship_vals):
    ...
    def search_or_create_partner(self, row, instance, partner_dict, country_dict, state_dict):
        ...
        if not country_obj:
            country_obj = self.env['res.country'].search_country_by_code_or_name(row.get('ship-country', ''))
            country_dict.update({row.get('ship-country', ''): country_obj})
        if recipient_name == 'CONFIDENTIAL':
            partner = res_partner_obj.search_or_create_amazon_partner(row, instance, country_obj)
        ...
        ship_vals.update({'state_id': state and state.id or False,
                          'country_id': country_obj and country_obj.id or False})
        if vat:
            ship_vals = self.check_and_update_vat_ept(row, instance, country_obj, vat_country, ship_vals)
            if not ship_vals.get('vat', ''):
                is_invalid_vat = True

        partner_dict, invoice_partner = self.search_or_create_amz_invoice_partner(
            row, instance, ship_vals, partner_dict)
    ...
    def request_shipment_report_by_software(self, seller_id, start_date, end_date):
        ...
        if not self.search([('start_date', '=', start_date),
            ...
            shipment_report = self.create({'report_type': ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA,
                                           'seller_id': seller_id,
                                           'state': 'draft',
            ...
            shipment_report.with_context({'is_auto_process': True,
                                          'emipro_api': 'shipping_request_report_v13'}).request_report()
    ...
    def search_or_create_amz_shipment_report(self, seller_id, report):
        ...
        :return:
        """
        report_id = report.get('GeneratedReportId', {}).get('value', False)
        request_id = report.get('ReportRequestId', {}).get('value', False)
    ...
    def process_and_create_amz_shipment_report(self, seller, start_date, end_date):
        ...
        for reports in list_of_wrapper:
            if not isinstance(reports.get('ReportRequestInfo', []), list):
                reports_list.append(reports.get('ReportRequestInfo', []))
            else:
                reports_list = reports.get('ReportRequestInfo', [])
                reports_list.reverse()
            for report in reports_list:
                self.search_or_create_amz_shipment_report(seller.id, report)
        return True
        ...
        if seller_id:
            ...
            for report in ship_reports:
                ...
                if report.report_id and report.state == '_DONE_' and not report.attachment_id:
                    report.with_context({'is_auto_process': True, 'amz_report_type': 'shipment_report'}).get_report()
    ...
    def get_reports_from_other_softwares(self, seller, start_date, end_date):
        ...
        kwargs.update({'emipro_api': 'get_shipping_or_inventory_report_v13',
                       ...
                       'end_date': end_date})
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
        if not response.get('reason', False):
    ...
    def check_fba_warehouse_partner_and_country_configured(self):
        ...
        missing_warehouses = self.seller_id.amz_warehouse_ids.filtered(lambda warehouse: not warehouse.partner_id.country_id)
